...
(async function exportVisible(tab,ticket,controls,memory){
  ...
  const entry=memory[ticket.attemptId||ticket.ticketId]||={attempted:false};
  ...
  const result=(stage,extra={})=>({ticketId:ticket.ticketId,attemptId:ticket.attemptId,stage,clickAttempted:entry.attempted,diagnostics:diagnostics(),...extra});
  ...
  if(!ticket.outputVideoNodeId){
    ...
    if(ticket.allowClick&&!entry.attempted){
      ...
      const dom=await tab.playwright.evaluate(()=>({text:document.body.textContent}));
      ...
      if(await button.count()!==1||!await button.isEnabled())return result('needs-export-control');
  ...
  return result(ready.length?'metadata-ready':errors.length?'video-error':ticket.outputVideoNodeId?'metadata-pending':exportControlReturned?(entry.attempted?'render-complete':'click-status-unknown'):'rendering-or-dialog',
